...
Official OpenRouter documentation confirms non-streaming Chat Completions at `https://openrouter.ai/api/v1/chat/completions`, strict `response_format.type=json_schema` with `json_schema.strict=true`, and `provider.require_parameters=true`. Image generation uses the current dedicated `POST https://openrouter.ai/api/v1/images` contract, not Chat Completions. Before each live image request, one client instance performs bounded authenticated discovery through `GET /api/v1/images/models` and the selected model's declared `GET /api/v1/images/models/{model}/endpoints` path; the result is cached for that publication run. The configured model must exist and one endpoint must support `resolution=1K` and `aspect_ratio=3:2`; `n=1` is used when advertised, while omission safely relies on the dedicated API's one-image default. `output_format=webp` is requested only when explicitly advertised. Optional compression/background are sent only when applicable and advertised, and provider routing is pinned without fallback. The official model catalog currently contains both requested exact IDs:
...
No substitution is made. If the image model or required resolution/aspect capabilities are absent, generation fails safely into `generation_failed`. Provider WebP support is not required: PNG, JPEG, or WebP input is locally normalized to the publication WebP contract. References: [API](https://openrouter.ai/docs/api/reference/overview), [structured outputs](https://openrouter.ai/docs/features/structured-outputs), [image generation](https://openrouter.ai/docs/features/multimodal/images), [image models](https://openrouter.ai/api/v1/images/models), and [models API](https://openrouter.ai/api/v1/models).
...
TLDR_EDITORIAL_MAX_PROVIDER_IMAGE_BYTES=12000000
TLDR_EDITORIAL_MAX_IMAGE_PIXELS=20000000
...
The dedicated Images API receives top-level `model`, `prompt`, supported `n`, `resolution`, `aspect_ratio`, and only advertised optional output fields. Exactly one `data` entry with `b64_json` is accepted. Declared media type and magic bytes must agree; omitted media type is inferred from PNG, JPEG, or WebP magic. Pillow rejects malformed/animated/decompression-bomb/excessive-pixel input, validates 3:2 dimensions and the 12,000,000-byte provider limit, applies EXIF orientation, flattens alpha onto opaque white, converts to RGB, strips metadata, and deterministically encodes WebP at quality 82/method 6. Native WebP is normalized by the same policy. The final WebP is revalidated and limited to 2,000,000 bytes; only its bytes determine SHA/key and reach R2. Separate `0600` provider and final temporary files are always deleted, and provider PNG/JPEG is never stored or uploaded. Pillow is locked through Pipenv. Editorial, capability, and image JSON responses use strict byte-limited readers; the image response allowance derives from the provider-input limit plus bounded base64/JSON overhead.
...
Editorial hash covers canonical bounded dossier + editorial model + editorial prompt version + output-schema version. Offline consistency reconstructs that dossier from current normalized data and rejects stale hashes after title, summary, membership, configured model, or controlled-version changes. For artifacts with an illustration hash, it also reconstructs temporary visual-source IDs, exact current source titles/summaries, and current image configuration to validate the illustration hash. Illustration hash covers validated brief + selected titles/summaries + image model + illustration prompt version + image configuration, including accepted upstream formats, conversion implementation version, RGB/quality-82/method-6/metadata/animation/background policy, provider byte/pixel limits, and final WebP byte limit. Public base URL is excluded, so changing it only updates the derived URL. Matching hashes preserve bytes/mtime and make zero model/R2 calls. For `--require-live`, all call/write/regeneration paths still require a completely clean Git workspace. A proven no-op may instead accept either a clean workspace or exactly an unstaged tracked modification of the selected dated artifact and manifest (or an untracked new dated artifact plus the modified tracked manifest). The complete offline validator must pass; NUL-delimited Git status parsing rejects every unrelated, staged, deleted, renamed, copied, conflicted, symlinked, or temporary path before returning zero calls and no write. A failed hash is attempted once automatically; `--retry-image` explicitly retries only its image stage. `--force` and `--retry-image` are never cron flags. Prompt-version changes affect only dates explicitly processed; default is latest, with no historical backfill.
