...
def raster_bytes(fmt="PNG"):
 from PIL import Image
 out=io.BytesIO();Image.new("RGB",(600,400),(20,40,60)).save(out,format=fmt,quality=95);return out.getvalue()
class Live:
 def __init__(self,fail=False,image_data=None,media_type="image/png"):self.editorial_calls=0;self.image_calls=0;self.fail=fail;self.image_data=image_data or raster_bytes();self.media_type=media_type
...
class Storage:
 def __init__(self,base='https://assets.example.workers.dev'):self.calls=0;self.base=base;self.uploaded=None;self.upload_path=None
 def ensure_uploaded(self,path,*a,**k):self.calls+=1;self.uploaded=path.read_bytes();self.upload_path=path;return True
...
class GeneratorTests(unittest.TestCase):
 ...
 def test_live_png_converts_uploads_only_final_webp_then_noops(self):
  temp_root=Path(tempfile.gettempdir());before={p for pattern in ('tldr-provider-*','tldr-final-*') for p in temp_root.glob(pattern)}
  ...
  self.assertTrue(s.uploaded.startswith(b'RIFF'));self.assertFalse(s.uploaded.startswith(b'\x89PNG'));self.assertFalse(s.upload_path.exists())
  after={p for pattern in ('tldr-provider-*','tldr-final-*') for p in temp_root.glob(pattern)};self.assertEqual(after,before)
  artifact=json.loads((self.o/'2026'/'2026-07-21.json').read_text());sha='sha256:'+__import__('hashlib').sha256(s.uploaded).hexdigest();self.assertEqual(artifact['illustration']['sha256'],sha);self.assertTrue(artifact['illustration']['object_key'].endswith('/'+sha[7:]+'.webp'))
