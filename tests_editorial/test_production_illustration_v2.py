...
class ProductionV2MatrixTests(unittest.TestCase):
 ...
 def test_immutable_pre_migration_v1_fixture_is_read_only_and_valid(self):
  repo=Path(__file__).resolve().parents[1];root=repo/'tests_editorial/fixtures/production_v1/editorial';path=root/'2026/2026-07-21.json';before=path.read_bytes();self.assertEqual(validate_all(root,repo/'generated'),[]);self.assertEqual(path.read_bytes(),before);a=json.loads(before);self.assertEqual(a['prompt_versions']['illustration'],'1.0.0');self.assertNotIn('illustration_profile_id',a);self.assertNotIn('visual_brief_schema_version',a);self.assertEqual(a['illustration']['object_key'],'daily/2026/07/21/e590e7ab31802ac3f5c64e8bd37282e87ed642f0baed62ee7775988481417f8f.webp')
 def test_current_production_v2_and_historical_v1_fixture_validate_independently(self):
  repo=Path(__file__).resolve().parents[1];current=repo/'generated/editorial';historical=repo/'tests_editorial/fixtures/production_v1/editorial';self.assertEqual(validate_all(current,repo/'generated'),[]);self.assertEqual(validate_all(historical,repo/'generated'),[]);a=json.loads((current/'2026/2026-07-21.json').read_text());old=json.loads((historical/'2026/2026-07-21.json').read_text());self.assertEqual(a['illustration_profile_id'],'production-v2');self.assertEqual(a['visual_brief_schema_version'],'2.0.0');self.assertNotIn('illustration_profile_id',old)
