...
class ProductionV2MatrixTests(unittest.TestCase):
 ...
 def test_invalid_editorial_refresh_matrix_never_reaches_image_or_r2(self):
  mutations={'missing_editorial_idea':lambda v:v.pop('editorial_idea'),'missing_central_subject':lambda v:v.pop('central_subject'),'missing_relationship':lambda v:v.pop('visual_relationship'),'missing_composition':lambda v:v.pop('composition'),'missing_literals':lambda v:v.pop('literal_elements'),'invalid_abstraction':lambda v:v.__setitem__('abstraction_level','extreme'),'duplicate_forbidden':lambda v:v.__setitem__('forbidden_elements',['text','text']),'oversized_failures':lambda v:v.__setitem__('failure_modes',['x']*13),'extra_field':lambda v:v.__setitem__('extra','x'),'foreign_reference':lambda v:v.__setitem__('source_candidate_ids',['c999']),'whitespace':lambda v:v.__setitem__('editorial_idea','   ')}
