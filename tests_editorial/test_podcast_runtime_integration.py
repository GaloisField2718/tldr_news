...
from tools.tldr_podcast import (DEFAULT,DEFAULT_EPISODE_PROFILE_ID,PodcastError,generate,load_state,measure_audio,publish,profile_from_args,run_daily,tts_turn)
...
def script(digest,n=10):
 turns=[]
 for i in range(1,n+1):
  speaker='speaker_a' if i%2 else 'speaker_b';text=('Why does this matter? Conversational search changes how distribution works, but the effect may vary depending on how quickly it spreads through the industry. '+'context '*1).strip();turns.append({'turn_id':f't{i:03}','speaker':speaker,'text':text,'pause_after_ms':250})
 s={'schema_version':'1.0.0','publication_date':'2026-07-21','locale':'en-US','episode_title':'Daily Index','summary':'A grounded technology briefing.','source_artifact_sha256':digest,'estimated_duration_seconds':0,'speakers':{'speaker_a':{'role':'cohost'},'speaker_b':{'role':'cohost'}},'turns':turns};s['estimated_duration_seconds']=round(sum(len(x['text']) for x in turns)/15.5+sum(x['pause_after_ms'] for x in turns)/1000);return s
class FakePost:
...
class RuntimeIntegration(unittest.TestCase):
 ...
 def test_interruption_after_script_and_turn_seven_resumes(self):
  ...
  try:
   ...
   second=FakePost(s,self.bytes);self.run_generate(root,second);self.assertEqual((first.editorial,first.tts,second.editorial,second.tts),(1,0,0,10))
   ...
   second=FakePost(s2,self.bytes);self.run_generate(root2,second);self.assertEqual((first.tts,second.tts), (7,3));td2.cleanup()
  finally:td.cleanup()
 def test_retry_400_and_budget_boundary(self):
  td,root,digest,s=self.case()
  try:
   retry=FakePost(s,self.bytes,fail=('t005',500));self.run_generate(root,retry);self.assertEqual((retry.tts,retry.by_turn['t005']), (11,2))
   td2,root2,digest2,s2=self.case();bad=FakePost(s2,self.bytes,fail=('t005',400))
   with self.assertRaisesRegex(PodcastError,'unrecoverable'):self.run_generate(root2,bad)
   self.assertEqual((bad.tts,bad.by_turn['t005']), (5,1));self.assertNotIn('t006',bad.by_turn);td2.cleanup()
   with patch.dict(os.environ,{'TLDR_PODCAST_RUNTIME_ROOT':str(root/'runtime')}),patch('tools.tldr_podcast.git_head',return_value='test-head'):
    rd,state=load_state(root,'2026-07-21',digest,profile_from_args(False,1),DEFAULT_EPISODE_PROFILE_ID);state['planned_turns']=[f't{i:03}' for i in range(1,17)];state['tts_attempts']=32
 ...
 def test_full_mocked_bilingual_run_daily_is_atomic(self):
  ...
  try:
   ...
   for t in fr['turns']:t['text']="Pourquoi cela compte-t-il ? La recherche conversationnelle change la distribution parce que les réponses restent sur place, mais l'effet peut varier selon le secteur."
   fr['estimated_duration_seconds']=round(sum(len(x['text']) for x in fr['turns'])/15.5+sum(x['pause_after_ms'] for x in fr['turns'])/1000)
   audio=self.bytes
   class Multi:
   ...
   self.assertEqual((post.editorial,post.tts,storage.client.uploads),({'en':1,'fr':1},22,2));self.assertEqual(set(result['artifact']['languages']),{'en','fr'});self.assertEqual(result['artifact']['speaker_profile'],'headline-brief-v1');self.assertTrue((root/'generated/podcast/2026/2026-07-21.json').exists())
  finally:td.cleanup()
