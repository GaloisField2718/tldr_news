...
VISUAL_KEYS={"mode","source_candidate_ids","central_subject","visual_metaphor","composition","forbidden_elements","alt_text"}
...
def validate_plan(plan: dict, candidates: list[Candidate]) -> dict:
    ...
    if not isinstance(vb,dict) or set(vb)!=VISUAL_KEYS: raise EditorialError("plan_visual_properties")
    mode=vb["mode"]
...
def resolve_plan(plan: dict,candidates:list[Candidate]) -> dict:
    ...
    vb=plan["visual_brief"]
    return {"lead":lead,"front_page":fp,"section_order":plan["section_order"],"visual_brief":{"mode":vb["mode"],"sources":[ref(x) for x in vb["source_candidate_ids"]],**{k:vb[k] for k in ("central_subject","visual_metaphor","composition","forbidden_elements","alt_text")}}}
