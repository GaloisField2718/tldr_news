...
# Canonical episode-shape profiles. One definition drives prompt generation, normalization,
# validation, compression/repair, preflight, cost estimation, and artifact metadata: no
# duration/turn/character constant lives anywhere else. Production is pinned to
# headline-brief-v1 for the current single-headline input. daily-briefing-v1 (the future
# multi-news profile) is intentionally not registered here yet: its generation pipeline
# does not exist, and registering unused bounds would only invite drift.
@dataclass(frozen=True)
class EpisodeProfile:
 profile_id:str
 min_turns:int;max_turns:int;preferred_turns:tuple[int,int]
...
if len({p.profile_id for p in _REGISTERED_EPISODE_PROFILES})!=len(_REGISTERED_EPISODE_PROFILES):raise RuntimeError("duplicate_episode_profile")
EPISODE_PROFILES=MappingProxyType({p.profile_id:p for p in _REGISTERED_EPISODE_PROFILES})
DEFAULT_EPISODE_PROFILE_ID="headline-brief-v1"
def get_episode_profile(profile_id:str|None=None)->EpisodeProfile:
 try:return EPISODE_PROFILES[profile_id or DEFAULT_EPISODE_PROFILE_ID]
 except KeyError as exc:raise PodcastError("podcast_episode_profile_unknown") from exc
...
def source_facts(source:dict[str,Any])->str:
 ...
 return facts[:12000]
def validate_script(script:dict[str,Any],source_digest:str,profile:EpisodeProfile)->dict[str,Any]:
 ...
 turns=script["turns"]
 if not isinstance(turns,list) or not profile.min_turns<=len(turns)<=profile.max_turns or set(script["speakers"])!={"speaker_a","speaker_b"}:raise PodcastError("podcast_script_schema_invalid")
 ...
 for i,t in enumerate(turns,1):
  ...
  if any(w in low for w in ("because","means","works","uses","happens when","since ","due to","driven by","credits","attributed to","the mechanism","results in","leads to","stems from","depends on","that's why","which is why","explains why","the reason","helps explain","parce que","signifie","fonctionne","utilise","se produit quand","car ","en raison de","attribué à","le mécanisme","entraîne","découle de","dépend de","explique pourquoi","la raison")):explains.add(t["speaker"])
...
def editorial_prompt(source:dict[str,Any],digest:str,d:str,profile:EpisodeProfile)->str:
...
def script_json_schema(profile:EpisodeProfile)->dict[str,Any]:
 ...
 return {"type":"object","additionalProperties":False,"required":["schema_version","publication_date","locale","episode_title","summary","source_artifact_sha256","estimated_duration_seconds","speakers","turns"],"properties":{"schema_version":{"type":"string","enum":["1.0.0"]},"publication_date":{"type":"string"},"locale":{"type":"string","enum":["en-US","fr-FR"]},"episode_title":{"type":"string"},"summary":{"type":"string"},"source_artifact_sha256":{"type":"string"},"estimated_duration_seconds":{"type":"integer"},"speakers":{"type":"object","additionalProperties":False,"required":["speaker_a","speaker_b"],"properties":{"speaker_a":{"type":"object","additionalProperties":False,"required":["role"],"properties":{"role":{"type":"string","enum":["cohost"]}}},"speaker_b":{"type":"object","additionalProperties":False,"required":["role"],"properties":{"role":{"type":"string","enum":["cohost"]}}}}},"turns":{"type":"array","minItems":profile.min_turns,"maxItems":profile.max_turns,"items":turn}}}
def editorial_request(post:Callable[...,Any],key:str,prompt:str,profile:EpisodeProfile)->str:
 body={"model":EDITORIAL_MODEL,"messages":[{"role":"user","content":prompt}],"response_format":{"type":"json_schema","json_schema":{"name":"daily_index_podcast_script","strict":True,"schema":script_json_schema(profile)}}}
...
def generate_script(post:Callable[...,Any],key:str,source:dict[str,Any],digest:str,d:str,state:dict[str,Any],ceiling:float,profile:EpisodeProfile,persist:Callable[[],None]|None=None,diagnostic_dir:Path|None=None)->dict[str,Any]:
 ...
 for attempt in range(used,limit):
  ...
  if persist:persist()
  try:
   raw=editorial_request(post,key,prompt,profile)
   ...
   if "duration_invalid" in str(exc):
    lo_c,hi_c=profile.preferred_chars;rw_hi=round((lo_c+hi_c)/2)
    prompt=editorial_prompt(source,digest,d,profile)+f"\nThe previous draft was too long. Rewrite it to be SHORTER: cut sentences, remove repetition and secondary detail, and shorten individual turns. Add no new claim, fact, or detail. Target {lo_c}-{rw_hi} spoken characters (never above {hi_c}) across {profile.min_turns}-{profile.max_turns} turns, {profile.min_duration}-{profile.max_duration} seconds, with pause_after_ms at 150-250ms. Keep at least one turn per speaker asking a question, one explaining a mechanism, and one stating a limitation or nuance -- do not drop these roles while shortening. Return strict schema JSON only. Oversized draft:\n"+raw
   else:prompt+="\nRepair the previous JSON to satisfy every schema, rotation, and balance rule: each speaker must ask a question (\"?\"), explain a mechanism (e.g. \"because\"), and state a limitation or nuance (e.g. \"however\", \"limitation\", \"not necessarily\"); each speaker's share of spoken characters must stay within "+f"{round(profile.speaker_share_min*100)}-{round(profile.speaker_share_max*100)}%"+". Add no claims; return JSON only. Previous draft:\n"+raw
...
def assemble_episode(rd:Path,script:dict[str,Any],state:dict[str,Any],profile:EpisodeProfile)->Path:
 ...
 if not profile.min_duration<=m["duration_seconds"]<=profile.max_duration:final.unlink(missing_ok=True);raise PodcastError("podcast_final_duration_invalid")
...
def generate_bilingual(root:Path,d:str,key:str,post:Callable[...,Any]=requests.post,profile_id:str=DEFAULT_EPISODE_PROFILE_ID)->dict[str,Any]:
 source,digest=load_source(root,d);rd,edition=edition_state(root,d,digest)
 with date_lock(rd):
  results={}
  with language_scope("en"):results["en"]=generate(root,d,key,post=post,cost=1.0,profile_id=profile_id)
  french_canaries(root,d,key,post,digest)
  with language_scope("fr"):results["fr"]=generate(root,d,key,post=post,cost=1.0,profile_id=profile_id)
...
def bilingual_artifact(root:Path,d:str,digest:str,variants:dict[str,dict[str,Any]],profile_id:str)->dict[str,Any]:
 now=time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime());return {"schema_version":"1.1.0","publication_date":d,"status":"published","source_artifact_sha256":digest,"speaker_profile":profile_id,"languages":variants,"generated_at":now,"published_at":now}
def publish_bilingual(root:Path,d:str,storage:Any,public_get:Callable[...,Any]=requests.get,profile_id:str=DEFAULT_EPISODE_PROFILE_ID)->dict[str,Any]:
 profile=get_episode_profile(profile_id);source,digest=load_source(root,d);rd,edition=edition_state(root,d,digest);variants={}
 with date_lock(rd):
  prepared={}
  for language in LANGUAGES:
   with language_scope(language):
    tts_profile=profile_from_args(False,1.0);lrd,state=load_state(root,d,digest,tts_profile,profile.profile_id);final=lrd/"episode.mp3";metrics=state.get("audio_validation")
  ...
  doc=bilingual_artifact(root,d,digest,variants,profile.profile_id);out=artifact_path(root,d)
  if out.exists():
...
def run_daily(root:Path,d:str,key:str,storage:Any,post:Callable[...,Any]=requests.post,public_get:Callable[...,Any]=requests.get,profile_id:str=DEFAULT_EPISODE_PROFILE_ID)->dict[str,Any]:
 generated=generate_bilingual(root,d,key,post,profile_id);doc=publish_bilingual(root,d,storage,public_get,profile_id);return {"generated":{k:v["private_audio"] for k,v in generated.items()},"artifact":doc,"frontend_sync":"required_after_commit"}

def preflight(root:Path,d:str,cost:float,profile_id:str=DEFAULT_EPISODE_PROFILE_ID)->dict[str,Any]:
 source,digest=load_source(root,d);profile=get_episode_profile(profile_id);profile_from_args(False,cost);rd=runtime_dir(d)
 target=round(profile.preferred_duration*15.5);tts=estimate_cost(DEFAULT,target)
 if .20+tts>cost:raise PodcastError("podcast_estimated_cost_exceeds_ceiling")
 return {"ready":True,"paid_calls":0,"publication_date":d,"source_artifact_sha256":digest,"languages":["en-US","fr-FR"],"selected_model":DEFAULT.model,"voices":DEFAULT.voices,
 "episode_profile_id":profile.profile_id,
 "target_duration_seconds":profile.preferred_duration,
 "valid_duration_range_seconds":[profile.min_duration,profile.max_duration],
 "turn_range":[profile.min_turns,profile.max_turns],
 "preferred_turn_range":list(profile.preferred_turns),
 "spoken_character_range":[profile.min_chars,profile.max_chars],
 "preferred_spoken_character_range":list(profile.preferred_chars),
 "planned_editorial_requests":2,"maximum_editorial_calls":profile.max_editorial_calls,"maximum_repairs":profile.max_editorial_calls-1,
 "planned_french_canaries":2,
 "planned_tts_turns_per_language":f"{profile.min_turns}-{profile.max_turns}","maximum_tts_attempts_per_language":2*profile.max_turns,
 "conservative_editorial_cost_usd_per_language":.20,"conservative_tts_cost_usd_per_language":tts,
...
def install_local_script(root:Path,d:str,language:str,script:dict[str,Any],origin:str="local_editorial_authoring",profile_id:str=DEFAULT_EPISODE_PROFILE_ID)->dict[str,Any]:
 ...
 with language_scope(language):
  rd,state=load_state(root,d,digest,tts_profile,profile.profile_id);validate_script(script,digest,profile);validate_grounding(script,source);script_file=rd/"script.json";_atomic_json(script_file,script,True);state["script_status"]="complete";state["script_sha256"]=sha256(script_file);state["planned_turns"]=[t["turn_id"] for t in script["turns"]];state["script_origin"]=origin;save_state(rd,state);return state

def generate(root:Path,d:str,key:str,post:Callable[...,Any]=requests.post,cost:float=1.0,profile_id:str=DEFAULT_EPISODE_PROFILE_ID)->dict[str,Any]:
 profile=get_episode_profile(profile_id);source,digest=load_source(root,d);tts_profile=profile_from_args(False,cost);rd,state=load_state(root,d,digest,tts_profile,profile.profile_id)
 with date_lock(rd):
  script_file=rd/"script.json"
  if script_file.exists():script=json.loads(script_file.read_text());validate_script(script,digest,profile);validate_grounding(script,source)
  else:
   script=generate_script(post,key,source,digest,d,state,cost,profile,lambda:save_state(rd,state),rd/"diagnostics");_atomic_json(script_file,script,True);state["script_status"]="complete";state["script_sha256"]=sha256(script_file);state["planned_turns"]=[t["turn_id"] for t in script["turns"]];save_state(rd,state)
  for turn in script["turns"]:tts_turn(post,key,tts_profile.candidate,turn,rd,state,cost);save_state(rd,state)
  final=rd/"episode.mp3" if state["assembly_status"]=="complete" else assemble_episode(rd,script,state,profile);save_state(rd,state);return {"private_audio":str(final),"state":state}
...
def publish(root:Path,d:str,storage:Any,accept:bool,public_get:Callable[...,Any]=requests.get,profile_id:str=DEFAULT_EPISODE_PROFILE_ID)->dict[str,Any]:
 profile=get_episode_profile(profile_id);source,digest=load_source(root,d);tts_profile=profile_from_args(False,1.0);rd,state=load_state(root,d,digest,tts_profile,profile.profile_id)
 with date_lock(rd):
  ...
  url,_=upload_audio(storage,final,d,metrics);verify_public_audio(public_get,url,metrics);state["upload_status"]="verified";state["frontend_status"]="prepared_external_sync";script=json.loads((rd/"script.json").read_text());doc=public_artifact(d,script,digest,metrics,url,profile.profile_id);out=artifact_path(root,d)
  if out.exists():
