 ...
 for attempt in range(used,limit):
  ...
  try:
   ...
   if "duration_invalid" in str(exc):
    lo_c,hi_c=profile.preferred_chars
    prompt=editorial_prompt(source,digest,d,profile)+f"\nConcise rewrite only: preserve every essential source fact, remove repetition and secondary detail, add no claims, use {lo_c}-{hi_c} spoken characters and {profile.min_duration}-{profile.max_duration} seconds across {profile.min_turns}-{profile.max_turns} turns. Return strict schema JSON only. Oversized draft:\n"+raw
